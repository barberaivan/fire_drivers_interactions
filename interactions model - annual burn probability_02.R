...
d$vegetation_code <- d$vegetation_valdivian
...
data <- d[d$vegetation_valdivian > 1, ]
data <- data[!(data$dataset == "unburned" & data$burned == 1), ]
...
m1 <- bam(
  burned ~
    ...
    s(ndvi, k = 7, by = vegetation_class, bs = "cr") +
      ti(ndvi, fwi, k = 5) +
...
dsub$northing <- cos(dsub$aspect * (pi / 180))
...
ggsave("figures/climate-fuel-interactions_m1_img3.png", plot = p2,
