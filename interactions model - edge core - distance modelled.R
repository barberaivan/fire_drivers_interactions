...
cov_all <- c(
  ...
  "elevation", "slope", "aspect", "northing",
  "TPI300", "TPI1k", "TPI2k",
...
predictors_cont_dist <- c(predictors_cont, "dstnc_p")

# standardize predictors
d_burnable_z <- d_burnable
for(i in predictors_cont_dist) {
  d_burnable_z[, i] <- as.numeric(scale(d_burnable[, i]))
}

# get means and sd to unstandardize
data_scale <- data.frame(
  mean = apply(d_burnable[, predictors_cont_dist], 2, mean),
  sd = apply(d_burnable[, predictors_cont_dist], 2, sd)
) %>% as.matrix %>% t
...
pdata$class_bin <- 1
pdata$pair_id_all <- d_burnable$pair_id[1]
...
# here, coefficients for continuous variables are going to include an effect
# of the distance between pixels pairs.
...
d_burnable_zh$vegetation_class <- factor(d_burnable_zh$vegetation_class,
                                         levels = enes_veg$vegetation_class)
n_veg <- length(levels(d_burnable_zh$vegetation_class))
...
# design matrix multiplied by distance
X_dist <- X[, n_veg:ncol(X)] * d_burnable_zh$dstnc_p
...
K_dist <- K - n_veg + 1 # number of interaction coefficients for distance
...
sdata <- list(
  N = N, NF = NF, J = J, K = K, K_dist = K_dist, N_pairs = N_pairs,
  N_fire = fires_enes$ndvi,
  ends = ends, starts = starts,
  core_ids = core_ids, edge_ids = edge_ids,
  X = X, X_dist = X_dist, Z = Z, dist = d_burnable_zh$dstnc_p,
...
# # compile
smodel <- stan_model("interactions model - edge core - distance modelled.stan")
smodel <- stan_model("interactions model - edge core - distance modelled - exp.stan")
# sample
m_fwi_h <- sampling(
...
saveRDS(m_fwi_h, "interactions model - edge core - distance modelled_samples_exp.rds")
# saveRDS(m_fwi_h, "interactions model - edge core - distance modelled_samples_squared.rds")
m_fwi_h <- readRDS("interactions model - edge core - distance modelled_samples.rds")
# # vuela. hermoso. 792.786 / 60 = 13.2131 min
sm <- summary(m_fwi_h)[[1]]
summary(sm[, "n_eff"]) # OK; min = 528.3
summary(sm[, "Rhat"])  # OK; max = 1.0126

# d parameters
# pairs(m_fwi_h, pars = "d")
...
dimnames(pveg) <- list(summ = dimnames(pveg)[[1]],
                       vegetation_class = levels(d_burnable_zh$vegetation_class),
...
ggsave("figures/climate-fuel-interactions_clogis_m1_hierarchical-at-median_veg.png",
       width = 17, height = 10, units = "cm")

# Distance notes ----------------------------------------------------------

# Including distance did not change the results. They are exactly the same.
# However, use this model.